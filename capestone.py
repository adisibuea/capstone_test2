...
st.title("Produksi Listrik dari Batubara dan Dampaknya Terhadap Lingkungan")
...
tab1, tab2, tab3 = st.tabs(['Emisi', 'Listrik', 'Solusi'])
with tab1:
    # data pertama
    ...
    st.write("Angka di bawah ini mengacu pada emisi CO2 dalam metrik ton (MtCO2) dari pembakaran bahan bakar di sektor energi. Data tersebut tidak mencakup sumber emisi gas rumah kaca penting lainnya yang terkait dengan energi seperti kebocoran metana dari operasi minyak dan gas, yang lebih sulit diukur.")
    df = pd.read_csv("./CO2 emissions from fuel combustion, Indonesia.csv")
    ...
    with col1:
        ...
        formatted= round(latest_year["CO2"],2)
        st.metric("Emissions Mt CO2", value= formatted
                #   , delta=f'Year {latest_year["Year"].year}'
                  )
    ...
    with col2:
        ...
        st.metric("Trend Emisi CO2", value=f'Tahun {oldest_year["Year"].year} ke {latest_year["Year"].year}', delta= f'{growth_percentage:.2f}%')
    # teks container
    ...
    4. __Lain-lain:__ Layanan komersial dan publik, agrikultur, dan konsumsi akhir yang tidak dirinci""")
    # data kedua
    df2 = pd.read_csv("./CO2 emissions by sector, Indonesia, 2021.csv")
    ...
    df.loc[df['persentase'] < 1, 'Sektor'] = 'lain-lain'
    df = df.groupby('Sektor', as_index=False).agg({'Value': 'sum', 'persentase': 'sum'})
    df['persentase2'] = df['persentase'].round(2).astype(str)+'%'
    # dashboard metric
    col1, col2 = st.columns(2)
    with col1:
        st.metric(df.iloc[-0]["Sektor"], value=df.iloc[-0]['persentase2']
                #   , delta= df.iloc[-0]["Sektor"]
                  )
    with col2:
        st.metric(df.iloc[-5]["Sektor"], value=df.iloc[-5]['persentase2']
                #   , delta= df.iloc[-2]["Sektor"]
                  )
...
with tab2:
    ...
    1. __Coal:__ Menggunakan bahan bakar fosil seperti batu bara untuk memanaskan air menjadi uap. Uap tersebut kemudian digunakan untuk memutar turbin yang menghasilkan listrik.
    2. __Natural Gas:__ Menggunakan gas alam untuk menghasilkan listrik melalui pembakaran dalam mesin pembangkit.
    3. __Oil:__ Pembangkit ini menggunakan mesin diesel (BBM) untuk menghasilkan listrik melalui pembakaran bahan bakar diesel.""")
    st.markdown("""
    ...
    1. __Solar PV:__ Mengubah energi matahari menjadi listrik menggunakan sel surya (panel surya).
    2. __Wind:__ Mengonversi energi kinetik angin menjadi listrik melalui turbin angin.
    3. __Hydro:__ Menggunakan energi air aliran atau potensial untuk menggerakkan turbin dan menghasilkan listrik.
    4. __Waste:__ Menggunakan bahan organik seperti kayu, limbah pertanian, atau sampah organik untuk menghasilkan listrik.           
    5. __Geothermal:__ Memanfaatkan panas bumi dari dalam bumi untuk menghasilkan uap dan menggerakkan turbin.
    6. __Biofuels:__ Menggunakan bahan bakar minyak yang berasal dari bahan organik seperti minyak sawit""")
    # data ketiga
    pembangkit = pd.read_csv("./Electricity generation sources, Indonesia, 2021.csv")
    emisibbm= pd.read_csv("./Emissions from power generation by source, Indonesia, 2021.csv")
    ...
    col1, col2 = st.columns(2)
    with col1:
        st.metric(df.iloc[-0]["sumber"], value=grouped_df.iloc[-0]['persentase2'])
    with col2:
        st.metric(df.iloc[-2]["grouped_sumber"], value=grouped_df.iloc[-1]['persentase2'])
    # Membuat bar chart menggunakan Altair
    chart = alt.Chart(grouped_df).mark_bar().encode(
        x=alt.X('grouped_sumber:N', axis=alt.Axis(title='Sumber'), sort= '-y'),
    ...
    filtered_df4 = df[df['grouped_sumber'] == 'Renewable Energy']
    filtered_df4['persentase2'] = filtered_df4['persentase'].astype(str)+'%'

    chart2 = alt.Chart(filtered_df4).mark_bar().encode(
        x=alt.X('sumber:N', axis=alt.Axis(title='Sumber'), sort= '-y'),
        y=alt.Y('Value', axis=alt.Axis(title='Produksi Listrik GWh')),
        # colour= 'sumber:N',
        tooltip=['Value:Q', 'persentase']
        ).properties(
        width=500,
        title='Renewable Energy 2021')
    text = chart2.mark_text(align='center', baseline='bottom', dy=-5).encode(
    text='persentase2:N')
    # Menggabungkan chart dan teks
    final_chart4 = (chart2 + text).configure_axisX(labelAngle=0)
    col1, col2 = st.columns(2)
    with col1:
        st.altair_chart(final_chart2)
    with col2: st.altair_chart(final_chart4)
    ...
    # hubungan regresi
    sumber_listrik = pd.read_csv("./electricity generation sources in Indonesia.csv")
    karbon_listrik = pd.read_csv("./emissions2 from power generation by source in Indonesia.csv")
    ...
    col1, col2 = st.columns(2)
    with col1:
        st.metric(filtered_df3.iloc[-0]["sumber2"], value=filtered_df3.iloc[-0]['persentase2'])
    with col2:
        st.metric(filtered_df3.iloc[-2]["sumber2"], value=filtered_df3.iloc[-2]['persentase2'])
    ...
    # with midcol:
    #     st.altair_chart(final_chart2)
    filtered_df3.rename(columns={'sumber2': 'sumber'}, inplace=True)
    grouped_df.rename(columns={'grouped_sumber': 'sumber', 'persentase2':'persentase3'}, inplace=True)
    tabel= pd.merge(filtered_df3, grouped_df, on='sumber')
    tabel.rename(columns={'persentase_x': 'CO2 MtCO2', 'persentase_y':'Listrik GWh'}, inplace=True)
    df_melted = pd.melt(tabel, id_vars=['sumber','Value_x', 'Year', 'Units', 'Value_y', 'persentase2','persentase3'], var_name='persentase', value_name='Persentase')
    df_melted['persen']= df_melted['Persentase'].astype(str)+'%'
    
    # Membuat bar chart
    bar_chart = alt.Chart(df_melted).mark_bar().encode(
        y="Persentase:Q",
        color="sumber:N",
        x=alt.X("persentase:N", axis=alt.Axis(title='', labelAngle=0))
    ...
    # Menggabungkan chart dan teks sebelum faceting
    final_chart3 = (bar_chart + text)

    # Menerapkan faceting
    final_chart3 = final_chart3.facet(column="sumber:N", columns=1)
    ...
    st.markdown("""
    Penggunaan batubara sebagai sumber energi untuk pembangkit listrik masih berlanjut di beberapa bagian dunia dengan beberapa alasan utama. Berikut beberapa alasan mengapa batubara masih digunakan:
    1. __Ketersediaan dan Harga Relatif Rendah:___ Batubara adalah sumber energi yang relatif murah dan tersedia secara melimpah di beberapa wilayah. 
    2. __Infrastruktur yang Ada:__ Banyak negara telah menginvestasikan sejumlah besar uang dalam infrastruktur pembangkit listrik berbasis batubara. 
    3. __Kebutuhan Energi yang Terus Meningkat:__ Sebagian besar negara masih menghadapi kebutuhan energi yang terus meningkat seiring pertumbuhan ekonomi dan populasi.
    4. __Ketidakpastian Pasokan Energi Terbarukan:__ Sumber energi terbarukan seperti angin dan surya bersifat fluktuatif dan bergantung pada kondisi cuaca.
    """)
    st.write("Dunia sangat mengandalkan batubara sebagai sumber energi untuk menghasilkan listrik. Pembakaran batu bara menghasilkan emisi CO2 yang signifikan. Meskipun batubara masih digunakan, ada tren menuju energi terbarukan dan upaya global untuk mengurangi ketergantungan pada sumber energi fosil. Semakin meningkatnya listrik yang dihasilkan berakibat ke emisi yang ikut meningkat")
    ...
    # text='persen:N')
    # # Menggabungkan chart dan teks
    # final_chart3 = (bar_chart + text).configure_axisX(labelAngle=0)
    # final_chart3.facet(column="sumber:N", columns=1)
    # st.altair_chart(final_chart3)
...
with tab3:
    ...
    st.write("Salah satu penyebab perubahan iklim disebabkan oleh meningkatnya emisi CO2 setiap tahunnya. Sektor yang menyumbang emisi terbesar adalah sektor pembangkit listrik. Hal ini dikarenakan sumber penggunaan energi mayoritas masih menggunakan energi fosil. Sumber energi penyumbang terbesar adalah batubara yang menyumbang 84% dari total emisi pembangkit listrik. Beberapa solusi yang dapat dilakukan")
    st.markdown("__Beralih ke pembangkit energi terbarukan__")
    st.markdown("""
    Beralih ke sumber energi terbarukan seperti matahari, angin, dan air dapat mengurangi emisi yang dihasilkan dari pembakaran bahan bakar fosil. Berikut beberapa faktor utama yang dapat memperlambat transisi ke energi terbarukan:
    1. __Infrastruktur yang Ada:__ Mengganti infrastruktur yang ada dengan teknologi terbarukan memerlukan waktu, sumber daya, dan investasi yang signifikan.
    2. __Biaya Awal yang Tinggi:__ Meskipun biaya operasional energi terbarukan dapat lebih rendah dalam jangka panjang, biaya awal untuk membangun pembangkit listrik terbarukan seperti panel surya, turbin angin, atau pembangkit listrik tenaga air cenderung tinggi.
    3. __Ketidakpastian Regulasi:__ Ketidakpastian dalam regulasi dan kebijakan pemerintah dapat membuat investor enggan untuk berkomitmen pada proyek energi terbarukan.
    4. __Pengaruh Industri Fosil:__ Industri energi konvensional, terutama batubara, gas, dan minyak, memiliki kepentingan ekonomi dan politik yang besar.""")
...
def main():
    ...
    st.sidebar.write(f"Setiap kenaikan satu unit dalam tingkat produksi listrik (GWh) akan diikuti oleh kenaikan {slope:.3f} unit dalam tingkat polusi (MtCO2). Ketika tingkat produksi listik (GWh) adalah nol, tingkat polusi (MtCO2) adalah {intercept:.2f}")
